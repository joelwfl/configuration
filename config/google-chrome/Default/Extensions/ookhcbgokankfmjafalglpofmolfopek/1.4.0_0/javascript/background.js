...
CanvasAnimation.prototype.drawFinal = function() {
  ...
  var nextEvent = JSON.parse(localStorage.nextEvent);
  if (!nextEvent) {
    this.showLoggedOut();
  } else {
    this.drawIconAtRotation();
    ...
    var nextEvents = JSON.parse(localStorage.nextEvents);
...
CalendarManager.pollServer = function() {
  if (! pollUnderProgress) {
    ...
    pendingLoadId_ = null;
    localStorage.calendars = JSON.stringify([]);
    localStorage.lastPollTime = getCurrentTime();
    var url;
...
CalendarManager.getCalendarFeed = function(calendarId) {
  ...
  try {
    ...
    xmlhttp.open('GET', JSON.parse(localStorage.calendars)[calendarId]);
    xmlhttp.send(null);
  }
  catch (e) {
    console.log('ex: ' + e);
...
CalendarManager.parseCalendarEntry = function(responseXML, calendarId) {
  ...
  if (calendarId < JSON.parse(localStorage.calendars).length) {
    CalendarManager.getCalendarFeed(calendarId);
  } else {
    CalendarManager.populateLatestEvent(JSON.parse(localStorage.eventList));
  }
};
...
CalendarManager.populateLatestEvent = function(eventList) {
  ...
  if (eventList.length > 0) {
    nextEvent = JSON.parse(localStorage.nextEvent);
    ...
    localStorage.nextEvents = JSON.stringify(nextEvents);
...
function redraw() {
  ...
  if (getCurrentTime() - localStorage.lastPollTime >= POLL_INTERVAL) {
    CalendarManager.pollServer();
  }
};
